...
class HandleListPage
{
    ...
    public function __invoke(\Bunny\Message $message, Channel $channel, Client $bunny): void
    {
        ...
        //TODO: Handle malformed messages -> nack, reque false, error log.
        ...
        Promise\all($pending)->then(
            ...
            function (array $extracted) use ($channel, $message) {
                ...
                $this->publishNextListPage($nextUrl, $message->getResource(), $channel)
                    ...
                    ->done(
                        null,
                        fn($reason) => $this->logger->error("Next URL publish failed: " . ReasonToString::convert($reason))
                    );
                ...
                $this->banot->items()->needScraping($detailsUrls)
                    ->then(function (ItemsNeedScrapingResponse $response) use ($channel, $message) {
                            $p = [];
                            foreach ($response->getUrls() as $detailUrl) {
                                $body = "{$message->getResource()}|{$detailUrl}";
                                $p[] = $channel->publish($body, [], '', 'DPQ'); //TODO: separate publishing into a Handler
                            }
                    ...
                    ->done(
                        null,
                        fn($reason) => $this->logger->error("Error when publishing details: " . ReasonToString::convert($reason))
                    );
            }
        )->done();
    }
    ...
    public function publishNextListPage(?string $nextUrl, string $resource, Channel $channel): Promise\PromiseInterface
    {
        if ($nextUrl !== null) {
            ...
            $body = "{$resource}|{$nextUrl}";                       //TODO: Wrapper around message body -> update Message class to only deal with this.
            return $channel->publish($body, [], '', 'LPQ');         //TODO: separate publishing into a Handler
        }
    ...
    private function getDetailPage(string $url): Promise\PromiseInterface
    {
        ...
        return $this->browser->get($url)
            ->then(function(ResponseInterface $r) use ($start, $url): string {
