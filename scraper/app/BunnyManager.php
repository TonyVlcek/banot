...
class BunnyManager
{
    ...
    public function connect(int $prefetchCount = 0): PromiseInterface
    {
        return $this->client->connect()
            ...
            ->then(function (Channel $channel) use ($prefetchCount) {
                return $channel->qos(0, $prefetchCount)->then(fn () => $channel);
            });
    }

    /**
     * Channel becomes available after successful connection (@see BunnyManager::connect())
     *
     * @throws ChannelUnavailableException
     */
    public function getChannel(): Channel
    {
        if (!isset($this->channel)) //uninitialized
        {
            throw new ChannelUnavailableException('The channel is not yet available. Are you connected to broker?');
        }
    ...
    protected function consumeQueue($handler, string $queueName): PromiseInterface
    {
        ...
        return $this->getChannel()
            ...
            ->then(function (MethodBasicConsumeOkFrame $response) use ($queueName) {
